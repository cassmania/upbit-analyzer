...
function config() {
    ...
    return { url: env.PRIVATE_SUPABASE_URL, anon: env.PRIVATE_SUPABASE_ANON_KEY,
        owner: env.PRIVATE_OWNER_ID, key: Buffer.from(env.PRIVATE_ENCRYPTION_KEY, "hex"), origin };
}
...
async function verifyToken(token) {
    ...
    if (!user || user.id !== config().owner || !user.email_confirmed_at) throw new PrivateError(403, "OWNER_ONLY");
